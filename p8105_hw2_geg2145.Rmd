...
df_mrTrash = read_excel("./data/trash_new.xlsx", sheet = 1, 
...
```{r p2_prof_trash}
df_profTrash = read_excel("./data/trash_new.xlsx", sheet = 2, 
                        col_names = TRUE, skip = 1,
                        range = cell_cols("A:M")) %>%
  janitor::clean_names() %>% 
  drop_na() %>%
...
```{r p2_gwynnda}
df_gwynnda = read_excel("./data/trash_new.xlsx", sheet = 4, 
                        col_names = TRUE, skip = 1,
                        range = cell_cols("A:L")) %>%
  janitor::clean_names() %>% 
  drop_na() %>%
...
**Gwynnda collected `r sum(pull(filter(df_gwynnda, month == "July" & year == 2021), cigarette_butts))` cigarette butts in July of 2021.**

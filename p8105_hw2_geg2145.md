...
trash put by one of these trash wheels into one of their dumpsters (845
rows total). There are 15, among which we have the unique
`dumpster`-`wheel` combination, the `weight_tons` collected, and the
`homes_powered` by that specific wheel.**

**Professor Trash Wheel collected 216.26 tons of trash total.**

**Gwynnda collected 3.109^{4} cigarette butts in July of 2022.**
...
disease. The amyloid 42/40 ratio holds significant promise for
...
**After importing the data with a standard `read_csv` call, I noticed
the first row described each column, so I had to skip that. I cleaned
all the variable names and changed the values of `sex` and `apoe4` to
“male”/“female” and “carrier”/“not carrier”, respectively. Since I
wanted the `age_on_onset` variable to be numerical, I exchanged all “.”
for NAs. Finally, I filtered out individuals who’s `age_on_onset` was
lower than their `current_age` (meaning they already had MCI when the
study started). After all that data cleaning, we had 479 total
individuals recruited, out of which 93 developed MCI. The average
baseline age was 65.0286013 and 30 percent of women are APOE4
carriers.**
...
```
    ...
    ## ℹ In argument: `biomarker_value = case_when(years_since_baseline == "Na" ~ NA,
    ##   TRUE ~ as.numeric(biomarker_value))`.
    ## Caused by warning:
    ## ! NAs introduced by coercion

**The dataset was imported using `read_csv` and skipping the first row
(since it contained descriptions of the data). After cleaning the data,
we have three variables: the individual’s `id` and their
`biomarker_value` for a specific time in the study (baseline, 2 years, 4
years, 6 years, 8 years). Note we changed the id variable name from
`study_id` to `id` to facilitate future merging. Some data cleaning also
took place so that `biomarker_value` is can be numeric variable and
`years_since_baseline` can be a factor variable with 5 levels.**
...
**There are 8 individuals in the baseline dataset that we don’t have
longitudinal data for (ID’s 14, 49, 92, 179, 268, 304, 389, 412) and 16
individuals in the longitudinal dataset that we don’t have baseline data
for (ID’s 72, 234, 283, 380, 484, 485, 486, 487, 488, 489, 490, 491,
492, 493, 494, 495).**

**The final dataset has 2355 and 8. Every row is a
`id`-`years_since_baseline` combination. For every one of those
combinations, we have the individual’s `sex`, `education` level, `apoe4`
status (“carrier”/ “not carrier”) and `biomarker_value` at that time,
along with the age they had at the beggining of the study and the age
they had when they got MCI.**
